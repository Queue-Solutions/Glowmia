...
import { sendAgentMessageWithFallback, type AgentLanguage, type AgentModeHint } from '@/src/server/agent';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Record<string, unknown> | ApiError>) {
  ...
  try {
    const result = await sendAgentMessageWithFallback({
      ...
      modeHint: readModeHint(req.body?.modeHint),
    });

    console.info(`[api/agent/message] Responded via ${result.source} with status ${result.status}.`);
    return res.status(result.status).json(result.body);
  } catch (error) {
    console.error('[api/agent/message] Unhandled error', error);
    return res.status(500).json({
      detail: error instanceof Error ? error.message : 'Unable to process agent message.',
    });
  }
}
