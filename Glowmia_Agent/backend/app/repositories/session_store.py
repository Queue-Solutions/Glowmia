...
class SupabaseSessionStore:
    ...
    def get(self, session_id: str) -> ChatSession | None:
        client = self._get_client()
        session_rows = client.table("chat_sessions").select("*").eq("id", session_id).limit(1).execute().data or []
        if not session_rows:
            return None
    ...
    def exists(self, session_id: str) -> bool:
        client = self._get_client()
        response = client.table("chat_sessions").select("id").eq("id", session_id).limit(1).execute()
    ...
    def append(self, session_id: str, message: ChatMessage) -> ChatSession:
        ...
        client.table("chat_sessions").update({"updated_at": now, "last_message_at": now}).eq("id", session_id).execute()
