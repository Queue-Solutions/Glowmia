...
class RecommendationService:
    ...
    def _run_with_reconnect(self, operation):
        ...
        raise RuntimeError("Supabase catalog is temporarily unavailable") from last_error
    ...
    def recommend(self, message: str, language: Language) -> list[Dress]:
        ...
        rows = self._run_with_reconnect(lambda client: client.table("dresses").select("*").limit(30).execute().data or [])
    ...
    def get_dress_by_id(self, dress_id: str, language: Language) -> Dress | None:
        rows = self._run_with_reconnect(
            lambda client: client.table("dresses").select("*").eq("id", dress_id).limit(1).execute().data or []
        )
        if not rows:
            return None
